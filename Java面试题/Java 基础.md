...
这主要是为了满足Java虚拟机的调用需要。
- public 是为了虚拟机更方便的调用main方法，而不用打破封装
- static 是为了在调用时不需要额外的创建对象
- void 是因为java 虚拟机不需要这个返回值，但是C、C++是又返回值的，这是因为C、C++程序可能被外部程序直接调用，这是需要通过这个返回值来判断调用的结果。