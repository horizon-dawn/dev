...
反射机制是代码在运行时动态获取类的信息的机制。允许在运行时获取构造方法、字段、方法和创建对象。反射之所以慢是因为反射可以绕过一些编译器的优化（比如JIT优化），使用反射时参数会包装成Object数组，方法执行是又会将Object数组转为真正的类型，会产生许多的中间对象，多想一多就容易出发GC，GC会导致程序变慢。反射会从数组中查询类的信息，并且进行检查。
