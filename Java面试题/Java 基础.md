...
graph TB
    subgraph Java特性
        ...
        J1 --> J1A[通过JVM实现<br/>一次编写到处运行]
        J2 --> J2A[垃圾回收器<br/>自动管理内存]
        J3 --> J3A[传递引用的副本]
        J4 --> J4A[避免菱形继承问题]
        J5 --> J5A[更安全但控制力弱]
        J6 --> J6A[编译成字节码<br/>JVM解释执行]
    end
    ...
    subgraph C++特性
        ...
        C1 --> C1A[需要针对不同<br/>平台编译]
        C2 --> C2A[使用new/delete<br/>手动管理]
        C3 --> C3A[值传递/指针传递<br/>引用传递]
        C4 --> C4A[单继承+多继承]
        C5 --> C5A[可操作内存和硬件]
        C6 --> C6A[直接编译成<br/>机器码]
    end
    ...
    subgraph 优缺点对比
        direction LR
        Java优点[Java优点:<br/>✓ 平台无关性强<br/>✓ 生态丰富<br/>✓ 降低内存泄露风险<br/>✓ 社区支持完善]
        Java缺点[Java缺点:<br/>✗ 启动速度较慢<br/>✗ 内存占用较大<br/>✗ 性能不如C++<br/>✗ 底层控制能力弱]
        
        C++优点[C++优点:<br/>✓ 性能高效<br/>✓ 直接操作硬件<br/>✓ 图形音频库丰富<br/>✓ 精细控制内存]
        C++缺点[C++缺点:<br/>✗ 内存管理复杂<br/>✗ 学习曲线陡峭<br/>✗ 容易内存泄露<br/>✗ 跨平台困难]
    end
    
    style Java特性 fill:#e1f5ff
    style C++特性 fill:#fff4e1
    style 优缺点对比 fill:#f0f0f0
```
...
- 4.9 怎么修改一个类中的 private 修饰的 String 参数的值？
    
- 4.10 JDK 9 中对字符串拼接做了什么优化？
...
- 14.1 ClassNotFoundException 和 NoClassDefFoundError 的区别是什么？
    
- 14.2 Java 中创建对象有哪些种方式？
    

### 十五、定时任务

- 15.1 Java 中 Timer 实现定时调度的原理是什么？
...
- 16.7 什么是 UUID，能保证唯一吗？
    
- 16.8 String 中 intern 的原理是什么？
    
- 16.9 什么是序列化与反序列化？
    
- 16.10 为什么建议多用组合少用继承？
    
- 16.11 BigDecimal 和 Long 表示金额哪个更合适，怎么选择？
    

---

请问你还需要对目录的其他部分进行修改，或者对其中某个问题进行解答吗？
## 一、Java 语言特性
...
**UUID** (Universally Unique Identifier)，即全局唯一标识符，是一个 128 位（16 字节）的数字，用于在分布式系统中对信息进行唯一标识。它也被称为 **GUID** (Globally Unique Identifier)。
