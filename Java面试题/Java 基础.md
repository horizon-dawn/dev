...
反射虽然会打破封装，但是还达不到矛盾的地步。封装、继承、多态是面向对象的三大特征，封装对内隐藏实现细节对外暴露安全接口进行访问。反射是Java提供的再运行时动态获取、操作类的机制，虽然反射会导致程序性能下降，但是在一些特定场景下还是需要进行使用的。框架设计如Spring、Mybatis，序列化和反序列化、JDK动态代理等。

我们可以使用最小化反射原则，在必须使用反射时再使用反射。同时明确反射白名单，限定反射的范围。最后就是在CodeReview的时候进行堆反射的代码严格的代码审查。

## Integer a=1000，Integer b=1000，\==是什么结果，如果是100呢？

当对象a、b的值都是1000时，a\==b为true，当a、b的值为100时 a\==b 为false，因为Integer内怒有一个静态内部类 IntegerCache 对数值为 -128 ~ 127 之间的对象进行返回，第二次创建b是会返回缓存好的对象，又因为 \== 比较的是两个对象的引用地址。